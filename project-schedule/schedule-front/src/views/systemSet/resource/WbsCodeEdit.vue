<template>
  <div ref="pageDiv" class="clearfix codeEditDiv">
    ...
    <el-dialog id="editRuleCodeDialog" :visible.sync="editPageVisble" width="40%" :show-close="true" title="编辑前缀" :close-on-click-modal="true" @close="closeEditDialog">
      ...
      <el-row>
        <el-col :span="24">
          <!-- <el-form-item label="编号类型：" prop="codeKind" style="margin-left:18px;">
              <el-radio-group v-model="formData.codeKind">
                <el-radio :label="1">固定字符</el-radio>
                <el-radio :label="2">日期</el-radio>
                <el-radio :label="3">类型代码</el-radio>
                <el-radio :label="4">流水号</el-radio>
              </el-radio-group>
            </el-form-item> -->
          <el-tabs
            v-model="ruleTab"
          ...
          >
            ...
            <el-tab-pane label="流水号" name="rulefour" :disabled="typeDisabled4">
              <el-form ref="formData" :model="formData" class="form-wrap" size="small" label-suffix="">
                <el-form-item label="位数：" prop="sortLength" style="margin-left:50px;margin-top:20px;">
                  <el-input-number v-model="formData.sortLength" controls-position="right" :min="1" :max="6" size="mini" style="width:150px" @change="updateInput" />
                </el-form-item>
                <el-form-item label="重置周期：" prop="reset" style="margin-left:20px;margin-bottom:20px;">
                  <el-select v-model="formData.reset" placeholder="重置周期" size="mini" style="width: 150px;" clearable>
                    <el-option
  ...
  methods: {
    ...
    toSave() {
      this.$refs['formInline'].validate((valid) => {
        if (valid) {
          ...
          //   this.listData[i].sortCode = i
          //   this.listData[i].wbsCodeId = this.wbsCodeId
          // }
    ...
    toConfirm() {
      ...
      if (_self.typeDisabled1 || _self.typeDisabled2 || _self.typeDisabled3 || _self.typeDisabled4) {
        ...
        // this.listData.forEach(rule => {
        //   rule.codeValue = changeCodeValue
        //   rule.sortLength = this.formData.sortLength
        //   rule.reset = this.formData.reset
        // })
      } else {
    ...
    closeEditDialog() {
      ...
      // this.resetForm('formDataOne')
      // this.resetForm('formDataTwo')
      // this.resetForm('formDataThree')
