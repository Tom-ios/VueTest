...
// 从树形数据中递归筛选目标值
function valInDeep(arr = [], val, id, childs, parent) {
  return arr.reduce((flat, item) => {
    let tmparr = []
    if (item[childs] && item[childs].length > 0) {
      tmparr = item[childs]
    } else if (item['children'] && item['children'].length > 0) {
      tmparr = item['children']
    }
    item.parent = parent
    return flat.concat(
      item[id] === val ? item : valInDeep(tmparr, val, id, childs, item)
    )
  }, [])
}
...
function arrayToTree(
  ...
  array.forEach(item => {
    ...
    const root = ['undefined', undefined, null].includes(item[options.pid])
