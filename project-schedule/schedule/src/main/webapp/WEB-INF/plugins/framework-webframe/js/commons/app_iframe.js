...
var App = function () {
    ...
    var requestFullScreen = function () {
        ...
        var de = document.getElementById('iframe_'+getActivePageId());
    ...
    return {
        ...
        getGlobalImgPath: function () {
            ...
            return basePath + '/fweb-framework-ui/style/images/';
        },
...
var addTabs = function (options) {
    ...
    //清除功能菜单边框
    $(".sidebar-menu a.nav-link").css('border','none');

    if (options.urlType == "relative") {
        // var url = window.location.protocol + '//' + window.location.host + "/";
        //var basePath = window.location.pathname + "/../";
        options.url = getBasePath4Fweb() + options.url;
    }
...
function closeTabByPageId(pageId) {
    ...
    if ($title.hasClass("active")) {
        //要关闭的tab处于活动状态
        //要把active class传递给前一个tab

        setTimeout(function () {
            //某种bug，需要延迟执行
        	if(prevActivePageIds.length>0){
        		var cloasedPageId = prevActivePageIds.pop();
        		prevActivePageIds = removeByValue(prevActivePageIds,cloasedPageId);
        		var prevActivePageId = prevActivePageIds.length>=1
        		? prevActivePageIds[prevActivePageIds.length-1]
        		: getPageId($(".page-tabs-content").children(":eq(1)"));
        		...
        		activeTabByPageId(prevActivePageId);
        	}
        }, 100);
    ...
    //如果可视区域小于
    if(visibleWidth > 
    		calSumWidth($title.nextAll(":visible"))
			+ calSumWidth($title.prevAll(":visible"))
		){
    	
    	if(!getLastTabStat()){
    		$title.nextAll(":hidden").each(function(index,e){
    			$(e).show();
    			if(visibleWidth <= 
	    			calSumWidth($title.nextAll(":visible"))
	    			+ calSumWidth($title.prevAll(":visible"))
    	...
    	if(!getFirstTabStat()){
    		$title.prevAll(":hidden").each(function(index,e){
    			$(e).show();
    			if(visibleWidth <= 
	    			calSumWidth($title.nextAll(":visible"))
	    			+ calSumWidth($title.prevAll(":visible"))
...
function closeTabOnly(pageId) {
    ...
    $title.remove();
    $tabPanel.remove();
    prevActivePageIds = removeByValue(prevActivePageIds,pageId);
}
...
var scrollToTab = function (element) {
    ...
    if(!$(element).is(":visible")){
    	...
    	$(".page-tabs-content").children(":first").show();
    	//如果当前标签宽度+当前标签后面所有标签标签宽度>可视区域，从右到左依次隐藏标签
    	if(visibleWidth <= 
	    		$(element).outerWidth(true)
	    		+ calSumWidth($(element).nextAll(":visible"))
	    		+ calSumWidth($(element).prevAll(":visible"))
    		){
    		var rightFirstvisibleTab = $(".page-tabs-content").children(":last");
    		while(visibleWidth <= 
    				$(element).outerWidth(true)
    				+ calSumWidth($(element).nextAll(":visible"))
    				+ calSumWidth($(element).prevAll(":visible"))
    			){
    			rightFirstvisibleTab.hide();
    			rightFirstvisibleTab = rightFirstvisibleTab.prev();
    		}
    	}else{
    		//否则，当前标签左侧依次显示，直到当前标签宽度+当前标签后面所有标签标签宽度+当前标签前面所有可视标签标签宽度
    		var leftFirstvisibleTab = $(element).prev();
    		while(visibleWidth >= 
	    			$(element).outerWidth(true)
	    			+ calSumWidth($(element).prevAll(":visible"))
	    			+ calSumWidth($(element).nextAll(":visible"))
    			){
    			leftFirstvisibleTab.show();
    			leftFirstvisibleTab = leftFirstvisibleTab.prev();
    		}
    		if(visibleWidth < 
	    			$(element).outerWidth(true)
	    			+ calSumWidth($(element).prevAll(":visible"))
    			...
    			leftFirstvisibleTab.next().hide();
    		}
    	}
    }else{
    	var leftVisibleTabWidth = calSumWidth($(element).prevAll(":visible"));
    	var rightVisibleTabWidth = calSumWidth($(element).nextAll(":visible"));
    	//可见标签+当前标签宽度大于等于标签可视区域，则从第一个标签开始隐藏，直到可见标签+当前标签宽度小于标签可视区域
    	if(visibleWidth <= $(element).outerWidth(true)+leftVisibleTabWidth+rightVisibleTabWidth){
    		var firstvisibleTab = $(".page-tabs-content").children(":first").next();
    		while(visibleWidth <= $(element).outerWidth(true)+calSumWidth($(element).prevAll(":visible"))+calSumWidth($(element).nextAll(":visible"))){
    			firstvisibleTab.hide();
    			firstvisibleTab = firstvisibleTab.next();
    		}
    	}else{
...
var scrollTabLeft = function () {
	...
	if(!getLastTabStat()){
		//获取显示区域的第一个元素
		var $firstVisibleTab = $(".page-tabs-content").children(".menu_tab:visible:last");
		$firstVisibleTab.next().show();
		$(".page-tabs-content").children(".menu_tab:visible:eq(1)").hide();
	}
};
//滚动条滚动到右边
var scrollTabRight = function () {
	...
	if(!getFirstTabStat()){
		//获取显示区域的第一个元素
		var $firstVisibleTab = $(".page-tabs-content").children(".menu_tab:visible:eq(1)");
		$firstVisibleTab.prev().show();
		$(".page-tabs-content").children(".menu_tab:visible:last").hide();
	}
};
...
function activeTabByPageId(pageId) {
	//保存激活Tab的pageId信息
	if(prevActivePageIds[length-1] != pageId){
		prevActivePageIds.push(pageId);
	}
...
//获取最后一个标签状态（true:显示	false:隐藏）
function getLastTabStat(){
	var lastTab = $(".page-tabs-content").children(":last");
	if(!lastTab || lastTab.size()==0)
		return false;
	return lastTab.is(":visible");
}

//获取第一个非首页标签状态（true:显示	false:隐藏）
function getFirstTabStat(){
	var firstTab = $(".page-tabs-content").children(":eq(1)");
	if(!firstTab || firstTab.size()==0)
		return true;
	return firstTab.is(":visible");
}
            ...
            $.each(data, function (i, item) {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    if (item.targetType != null && item.targetType === "blank") //代表打开新页面
                    {
                        ...
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,urlType:\'' + item.urlType + '\',url:\'' + item.url + '\'});';
                        $a.attr('onclick', href);
